...
def clean_path_input(raw_path):
    ...
    return raw_path.strip().rstrip(r"\/")
#for pytest(random):
def evaluate_toss(user_choice, winning_choice):
    if user_choice.lower() == winning_choice:
        return "Win"
    else:
        return "Lose"
 
#for pytest(systemdata):
def format_host_display(hostname):
    return str(hostname).upper().strip()

def main():
    global name, datas, alias
    # Starting
    print("""NOTE : ------------------------------------------------------------------
    ...
    while True:
      ...
      # random  
            ...
            #
            result = evaluate_toss(choiceinp,choice)
            if result == "Win":
      ...
      # cd [command]
            ...
            raw_path = inputs[3:].strip()
            #
            path = clean_path_input(raw_path)
            if not path:
     ...
     # file [Source,destination]      
      ...
      # systemdata        
            ...
            clean_host = format_host_display(hostname)
            print(f"Device: {clean_host}, Ip : {ip_address}")
            logging.info(f"Device: {clean_host}, Ip : {ip_address}")
      # cmd history
